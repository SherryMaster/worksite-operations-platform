...
create table public.migration_batches (
  ...
  file_checksum text not null
    check (file_checksum ~ '^[a-f0-9]{64}$'),
  status public.migration_batch_status not null default 'PREVIEWED',
...
create unique index migration_batches_one_committed_checksum
on public.migration_batches (file_checksum)
where status = 'COMMITTED';

create index migration_batches_created_at_idx
on public.migration_batches (created_at desc);

alter table public.migration_batches enable row level security;

grant select, insert on public.migration_batches to authenticated;

create policy "CEO can read migration batches"
on public.migration_batches
for select
to authenticated
using (private.is_current_ceo());

create policy "CEO can stage migration batches"
on public.migration_batches
for insert
to authenticated
with check (
