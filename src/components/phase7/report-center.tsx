...
export async function ReportCenter({
  ...
  const [loadedReport, projects, workers] = await Promise.all([
    part === "toolbar"
      ? Promise.resolve(null)
      : loadReport(parsed.reportId, parsed.filters),
    part !== "results" && usesProject
      ? listAttendanceProjects()
      : Promise.resolve([]),
    part !== "results" && usesWorker ? listWorkers() : Promise.resolve([]),
  ]);
  const report =
    loadedReport ??
    ({
        ...
        <section className="mt-4" aria-labelledby="report-result-title">
          ...
          {previewLimited ? (
            <p className="mt-3 border-l-2 border-violet-400 pl-3 text-xs text-slate-600">
              ...
              {report.rows.length.toLocaleString()} total rows. Narrow the
              filters or download the Excel file to review every row.
            </p>
          ) : null}
