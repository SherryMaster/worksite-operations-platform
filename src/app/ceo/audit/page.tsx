...
export default async function AuditPage({
  ...
  return (
    <main className="px-5 py-8 sm:px-8 lg:py-10">
      ...
      <form
        action="/ceo/audit"
        className="mt-6 grid gap-4 border border-violet-100 bg-white p-4 md:grid-cols-2 xl:grid-cols-[1.4fr_1fr_1fr_1fr_auto]"
      >
        <label className="space-y-2 text-xs font-semibold uppercase tracking-wider text-slate-500">
          ...
          <input
            ...
            autoComplete="off"
            className="mt-2 h-11 w-full border border-violet-100 bg-slate-50 px-3 text-sm font-normal normal-case tracking-normal text-slate-950 focus-visible:border-amber-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-600/20"
          />
        </label>
        <label className="space-y-2 text-xs font-semibold uppercase tracking-wider text-slate-500">
          Changed By
          <span className="relative mt-2 block">
            ...
            <input
              ...
              autoComplete="off"
              className="h-11 w-full border border-violet-100 bg-slate-50 pl-10 pr-3 text-sm font-normal normal-case tracking-normal text-slate-950 focus-visible:border-amber-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-600/20"
            />
          </span>
        </label>
        <label className="space-y-2 text-xs font-semibold uppercase tracking-wider text-slate-500">
          ...
          <span className="relative mt-2 block">
            <select
              ...
              className="h-11 w-full appearance-none border border-violet-100 bg-slate-50 px-3 pr-10 text-sm font-normal normal-case tracking-normal text-slate-950 focus-visible:border-amber-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-600/20"
            >
            ...
            <ChevronDown
              className="pointer-events-none absolute right-3 top-3.5 size-4 text-slate-500"
              aria-hidden="true"
            />
          </span>
        </label>
        ...
        <div className="flex items-end gap-2">
          <FormSubmitButton
            ...
            className="inline-flex h-11 flex-1 touch-manipulation items-center justify-center gap-2 bg-violet-700 px-5 text-sm font-semibold text-white hover:bg-violet-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-600 focus-visible:ring-offset-2"
          >
          ...
          {filtersActive ? (
            <Link
              href="/ceo/audit"
              className="inline-flex h-11 items-center border border-violet-100 px-4 text-sm font-semibold hover:border-violet-950 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-600"
            >
              Clear
            </Link>
          ) : null}
        ...
        {entries.length === 0 ? (
          ...
          <ol className="space-y-3">
            {entries.map((entry) => (
              <li
                key={entry.id}
                className="border border-violet-100 bg-white p-5 sm:p-6"
              >
                <div className="grid gap-5 lg:grid-cols-[11rem_minmax(0,1fr)_auto]">
                  ...
                  <div className="min-w-0">
                    <h2 className="text-pretty text-lg font-semibold text-slate-950">
                      {entry.presentation.title}
                    </h2>
                  ...
                  <span className="h-fit w-fit border border-slate-200 bg-slate-50 px-2.5 py-1 text-[0.65rem] font-semibold uppercase tracking-wider text-slate-600">
                    {entry.presentation.area}
                  </span>
                </div>
