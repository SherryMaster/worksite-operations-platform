...
import { PayrollStatementSkeleton } from "@/components/operations/loading-skeletons";
...
export default async function PayrollStatementPage({
  ...
  return (
    <main className="mx-auto max-w-5xl px-5 py-8 sm:px-8 print:max-w-none print:p-0">
      ...
      <Suspense fallback={<PayrollStatementSkeleton />}>
        <PayrollStatement statementPromise={statementPromise} />
      </Suspense>
    </main>
  );
}
              ...
              {buckets.map((bucket, index) => (
                <tr key={`${String(bucket.id)}-${index}`}>
                  <td className="px-4 py-3">
                    {String(bucket.category).toLowerCase().replaceAll("_", " ")}{" "}
                ...
                >
                  <div>
                    ...
                    <p className="mt-1 text-xs text-slate-500">
                      {String(day.day_type).toLowerCase().replaceAll("_", " ")}
                      {day.approved_leave
