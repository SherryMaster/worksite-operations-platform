...
async function loadSelectedMonthRows(
  projectsPromise: ReturnType<typeof listAttendanceProjects>,
  ...
  const month = /^\d{4}-\d{2}$/.test(params.month ?? "")
    ...
    : currentMonth();
  return projectId ? getAttendanceMonthRows(projectId, month) : [];
}
...
export default async function CeoAttendancePage({
  ...
  const dailyKey = [params.project, params.date]
    .map((value) => value ?? "")
    .join("|");
  const monthlySummaryKey = [params.project, params.month]
    .map((value) => value ?? "")
    .join("|");
  const monthlyRowsKey = [
    params.project,
    params.month,
    ...
    .map((value) => value ?? "")
    .join("|");
  const projectsPromise = listAttendanceProjects();
  const monthRowsPromise =
    view === "month"
      ? loadSelectedMonthRows(projectsPromise, params)
      : Promise.resolve([]);
  ...
  return (
    <main>
      ...
      <Suspense fallback={<DirectoryToolbarSkeleton filters={2} />}>
        <AttendanceContent
          monthRowsPromise={monthRowsPromise}
          params={params}
          part="controls"
      ...
      {view === "day" ? (
        <Suspense key={dailyKey} fallback={<AttendanceWorkspaceSkeleton />}>
          <AttendanceContent
            monthRowsPromise={monthRowsPromise}
            params={params}
            part="daily"
        ...
        <>
          <Suspense
            ...
            fallback={<AttendanceSummarySkeleton />}
          >
            <AttendanceContent
              monthRowsPromise={monthRowsPromise}
              params={params}
              part="monthly-summary"
              projectsPromise={projectsPromise}
            />
          </Suspense>
          <Suspense
...
async function AttendanceContent({
  ...
  monthRowsPromise: Promise<Awaited<ReturnType<typeof getAttendanceMonthRows>>>;
  params: AttendanceSearchParams;
  part: "controls" | "daily" | "monthly-results" | "monthly-summary";
  projectsPromise: ReturnType<typeof listAttendanceProjects>;
}) {
  const projects = await projectsPromise;
  const projectId = projects.some((project) => project.id === params.project)
  ...
  const view = params.view === "month" ? "month" : "day";
  const snapshot =
    part === "daily" && projectId && view === "day"
      ? await getAttendanceSnapshot(projectId, workDate)
      : null;
  const allMonthRows =
    part.startsWith("monthly") && projectId && view === "month"
      ? await monthRowsPromise
      : [];
      ...
      {part !== "controls" ? (
        <>
          {projects.length === 0 ? (
            ...
            <section className={part === "monthly-summary" ? "pt-4" : ""}>
              {part === "monthly-summary" ? (
                        ...
                        <summary className="flex min-h-20 cursor-pointer list-none items-center gap-3 px-3 py-2.5">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <p className="mt-1 text-xs tabular-nums text-slate-500">
                              {row.date} · {formatMinutes(row.totalMinutes)}{" "}
                              payable
                            </p>
                          </div>
                          <ChevronRight
                      ...
                      <tbody className="divide-y divide-slate-200">
                        ...
                        {monthRows.length === 0 ? (
                          <tr>
                            ...
                            >
                              No recorded attendance matches this month and
                              search.
                            </td>
                          </tr>
                        ) : null}
                  ...
                  {pageCount > 1 ? (
                    ...
                    >
                      <p className="text-xs text-slate-500">
                        ...
                        {filteredMonthRows.length} records
                      </p>
